 ...
 *
 */
public abstract class DepartsItemsEdit extends CashierBasePage{
	...
	public void queryDepartsItems(IRequestCycle cycle) throws Exception {
		...
		IDataset departs = paramsBean.queryDepartsItems(pd, params, null);
		if(departs!=null && departs.size() == 1){
			this.setInfo(departs.getData(0));
		}else{
			common.error("获取部门项目参数信息失败！");
		}
	}
	/**
	 * 修改项目信息
	 ...
	 * @date:2010-5-13
	 */
	public void saveDepartsItems(IRequestCycle cycle) throws Exception {
		PageData pd = this.getPageData();
		IData conditions = new DataMap();
		String msg = "";
		...
		if("edit".equals(operType)){
			...
			this.paramsBean.updateDepartsItems(pd, params);
			msg = "修改部门项目参数成功！";
		}
		//新增
		else if("add".equals(operType)){
			...
			dataset.add(params);
			this.paramsBean.addDepartsItems(pd, dataset);
			msg = "新增部门项目参数成功！";
		}
		//返回项目参数查询界面
