...
//    initialize: function(input, suggestArea, candidateList, displayList) {
	...
	//根据code值初始化text值
	this.setInputText(this._loadTextByCode(this.getCodeText()));
  ...
  _splitToList: function (srcList){
	  var descList = [];
	  for(var i = 0, length = srcList.length; i < length; i++){
		descList.push(srcList[i].split(":")[1]);
	  }
	  return descList;
  },
  _loadTextByCode: function (src){
	  for(var i = 0, length = this.codeList.length; i < length; i++){
		  if (this.codeList[i] == src){
 ...
 _changeToPinYinList: function(srcList){
  ...
  changeActive: function(index) {
	...
	this.setCodeText(this.codeList[this.suggestIndexList[index]]);
